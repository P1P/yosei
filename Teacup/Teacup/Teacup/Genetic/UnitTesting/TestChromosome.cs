...
namespace Teacup.Genetic.UnitTesting
{
    ...
    public class TestChromosome
    {
        ...
        public void InitGetSet()
        {
            ...
            Chromosome<int> chr_1 = new Chromosome<int>();
            ...
            Chromosome<decimal> chr_2 = new Chromosome<decimal>(data_array);

            for (int i = 0; i < data_array.Length; ++i)
            {
                Assert.AreEqual(chr_2.GetGene(i).get_data(), data_array[i]);
            }
        }
        ...
        {
            Chromosome<int> chr_base_1 = new Chromosome<int>(new int[] { 1, 2, 3, 4, 5 });
            Chromosome<int> chr_base_2 = new Chromosome<int>(new int[] { -1, -2, -3, -4, -5 });
            ...
            Assert.AreEqual(chr_1.GetGenesCopyArray(), chr_base_1.GetGenesCopyArray());
            Assert.AreEqual(chr_2, chr_base_2);
            ...
            Assert.AreEqual(chr_1, chr_base_2);
            Assert.AreEqual(chr_2, chr_base_1);
            Assert.AreNotEqual(chr_1, chr_base_1);
            Assert.AreNotEqual(chr_2, chr_base_2);
        ...
        public void Mutation_decimal()
        {
            ...
            Chromosome<decimal> chr_1 = new Chromosome<decimal>();

            for (int i = 0; i < nb_genes; ++i)
            {
                chr_1.AddGene(new Gene<decimal>(1m));
                Assert.AreEqual(chr_1.GetGene(i).get_data(), 1f);
            }
            ...
            for (int i = 0; i < nb_mutations_per_gene; ++i)
            {
                for (int j = 0; j < nb_genes; ++j)
                {
                    ...
                    Assert.AreEqual(chr_1.GetGene(j).get_data(), rng[j + i * nb_genes]);
                }
            }
        }
